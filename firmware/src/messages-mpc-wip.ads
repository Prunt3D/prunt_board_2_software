...
package Messages is
   ...
   type Heater_Kind is (Disabled_Kind, PID_Kind, Bang_Bang_Kind, PID_Autotune_Kind, MPC_Kind) with
     Size => 8;
   ...
   type Fixed_Point_Heat_Capacity is delta 2.0**(-18) range 0.0 .. 16_000.0 with
     Size => 32;

   type Fixed_Point_Thermal_Conductance is delta 2.0**(-27) range 0.0 .. 31.0 with
     Size => 32;

   type Fixed_Point_Power is delta 2.0**(-22) range 0.0 .. 1_000.0 with
     Size => 32;

   type Fixed_Point_Smoothing_Factor is delta 2.0**(-31) range 0.0 .. 1.0 with
     Size => 32;

     --  TODO: Get some measurements and tune this to suit all heater setups.
   type Fixed_Point_Frequency is delta 2.0**(-21) range 0.0 .. 2_000.0 with
     Size => 32;

   type Fixed_Point_Temperature_Over_Time is delta 2.0**(-25) range 0.0 .. 127.0 with
     Size => 32;
   ...
   type Fixed_Point_Length is delta 2.0**(-32) range 0.0 .. 0.99 with
     Size => 32;

   type Fixed_Point_Lengthwise_Heat_Capacity is delta 2.0**(-30) range 0.0 .. 3.99 with
     Size => 32;
   ...
   type Heater_Parameters (Kind : Heater_Kind := Disabled_Kind) is record
      ...
      case Kind is
         ...
         when MPC_Kind =>
            ...
            Ambient_Transfer               : Fixed_Point_Thermal_Conductance;
            Full_Fan_Ambient_Transfer      : Fixed_Point_Thermal_Conductance;
